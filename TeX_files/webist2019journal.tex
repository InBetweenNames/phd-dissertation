...
This paper has been published in the WEBIST 2019 Springer journal.
We were invited to submit this paper following our presentation at the WEBIST 2019 conference.
...
The triplestore contains triples such as those in \Cref{webist2019conf:evdiscover} which represent the event in which \textit{hall} (in the role of ``\property{subject}'') discovered \textit{phobos} (in the role of ``\property{object}'') in 1877 (in the role of ``\property{year}'') with the \texttt{refractor\_telescope\_1} (in the role of ``\property{implement}'')  at the \texttt{us\_naval\_observatory} (in the role of ``\property{location}'').  Events representing set membership are represented as shown in \Cref{webist2019conf:evmember}.
...
\noindent All of the events of type ``$\opit{discover}$'' are collected together and are checked to see if $\entityassoc{phobos}$ is found as the \property{object} role value of any of them. If so \textit{True} is returned.
...
\noindent No \property{subject}, or prepositional phrase is given in the query, and so all events of type ``$\opit{discover}$'' are considered and filtered by the denotation of the \property{object} termphrase ``\texttt{io}'' and then, those that pass the filter are returned by the verbphrase and the denotation of the word ``\texttt{where}'' picks out the location from those events. %(See section $\Cref{sec:examples}$ for more details).
...
\noindent No \property{subject} or \property{object} is in the query so all events of type ``$\opit{discover}$'' are considered and only those with the \property{year} property equal to 1610 pass the filter and then the denotation of the word ``\texttt{who}'' selects the subject which is returned.
...
\noindent This would retrieve the triples of all ``$\opit{discover}$'' events that contain a \property{subject} property, including the triples describing the type of those events.  The second function is:
...
In the second case, ``\texttt{in 1877}'' applies to ``\texttt{discovered}'', a relation which has the concept of a time of discovery (the \property{year} role).  As \textit{hall} is the only person that discovered anything in 1877, only they are included in the result.
