...
\examplequery{a moon was discovered with every telescope}{False} (No single moon was discovered using every telescope in our database)
...
discovery event has 5 roles: ``\property{subject}'' (agent), ``\property{object}'' (theme), ``\property{implement}'', ``\property{year}''
and ``\property{location}''. The triplestore defines 20 binary relations between these 5 roles: $\opit{subject} \rightarrow \opit{object}$,
...
\item \examplequerynl{which moons were discovered with two telescopes}{halimede laomedeia sao themisto}

\item \examplequerynl{who used two telescopes to discover a moon}{nicholson science\_team\_18 science\_team\_2}

\item \examplequery{who discovered sao}{science\_team\_18}

\item \examplequery{who discovered themisto}{science\_team\_2}

\item \examplequery{which moon was discovered by science\_team\_18 with two telescopes}{halimede laomedeia sao}

\item \examplequerynl{what was used to discover sao}{blanco\_telescope canada-france-hawaii\_telescope}

\item \examplequerynl{what did nicholson discover with two telescopes}{sinope lysithea carme ananke}

\item \examplequerynl{which moon was discovered by nicholson with two telescopes}{none}

\item \examplequerynl{which moon was discovered by nicholson with one telescope}{ananke carme lysithea sinope}

\item \examplequery{how was sinope discovered}{refractor\_telescope\_2}

\item \examplequery{how was carme discovered}{hooker\_telescope}

\item \examplequery{how was ananke discovered}{hooker\_telescope}

\item \examplequery{how was lysithea discovered}{hooker\_telescope}

\item \examplequery{what did nicholson discover with one telescope}{nothing}

\item \examplequerynl{what did nicholson discover with a telescope}{sinope lysithea carme ananke}
\end{enumerate}
...
\begin{enumerate}
	...
	that are \property{subject}s of events of type ``$\opit{member}$'' whose object is ``$\opit{person}$''. Our parser
