...
The calculation here involves computing $\FDBR{\relation{discover}\_\mathit{evs}_{\mathit{subject}}}$, then filtering it with the denotation of ``in 1948 and 1949'', then finally filtering it with the denotation of ``with a telescope''. %(reference to be provided after review process) %Peelar's Master's Thesis \cite{peelar2016accommodating} explains this in more detail.
...
%	\item who discovered sinope – $\mathit{nicholson}$
...
%	\item which moon was discovered with two telescopes by nicholson – $\mathit{None}$
%	\item which moon was discovered by nicholson with two telescopes – $\mathit{None}$
%\end{enumerate}
%
