...
%TODO: at end
In \Cref{subsec:prevwork} we describe previous work on NLQIs to the Semantic Web that support negation.  In \Cref{sec:demo} we describe how to access a live demonstration of our NLQI that can accommodate the queries presented in this paper along with some other example queries.  In \Cref{sec:event} we describe our event-based semantics and in \Cref{sec:negation} we describe how to accommodate negation where the CWA holds. In \Cref{sec:examples} we provide a list of examples queries and explain how they are processed.  Finally, we conclude in \Cref{sec:future} and \Cref{sec:conclusion}.
...
\noindent In all cases, the query processor returns the syntax tree of the query to help the user understand how the query was evaluated \cite{peelar2020compositional}.  A list of example queries and a discussion of how they are evaluated can be found in \Cref{sec:examples}.
